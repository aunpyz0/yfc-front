            ...
            >
                <template v-slot:top>
                    <v-toolbar
                        flat
                    >
                        <v-toolbar-title>ธนาคารทั้งหมด</v-toolbar-title>
...
export default {
    ...
    async mounted() {
        try {
            this.items = await this.$axios.$get('/banks')
        } catch (err) {
            console.error(err)
        }
    },
    methods: {
        ...
        async save() {
            try {
                if (this.editedItem.id) {
                    const updated = await this.$axios.$put(`/banks/${this.editedItem.id}`, {
                        id: this.editedItem.id,
                        name: this.editedItem.name,
                    })
                    this.items = this.items.map(item => item.id === updated.id ? updated : item)
                } else {
                    const created = await this.$axios.$post('/banks', {
                        name: this.editedItem.name,
                    })
        ...
        async confirmDelete() {
            try {
                await this.$axios.$delete(`/banks/${this.editedItem.id}`)
